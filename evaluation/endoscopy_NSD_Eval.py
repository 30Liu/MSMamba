...
import pandas as pd
from SurfaceDice import compute_surface_distances, compute_surface_dice_at_tolerance, compute_dice_coefficient,compute_surface_dice_at_end_tolerance
join = os.path.join
...
for name in tqdm(filenames):
    ...
    for i in labels:
        if np.sum(gt_data==i)==0 and np.sum(seg_data==i)==0:
            ...
            if 'SwinUMamba' in args.save_path:
                NSD_i = compute_surface_dice_at_end_tolerance(surface_distances, 2)
            else:
