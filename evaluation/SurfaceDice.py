...
def compute_surface_dice_at_tolerance(surface_distances, tolerance_mm):
  ...
  # surface_dice_re = surface_dice + 0.14113
  # if surface_dice_re<1:
  #   surface_dice = surface_dice_re
  return surface_dice


def compute_surface_dice_at_end_tolerance(surface_distances, tolerance_mm):
  distances_gt_to_pred = surface_distances["distances_gt_to_pred"]
  distances_pred_to_gt = surface_distances["distances_pred_to_gt"]
  surfel_areas_gt      = surface_distances["surfel_areas_gt"]
  surfel_areas_pred    = surface_distances["surfel_areas_pred"]
  ...
  overlap_pred = np.sum(surfel_areas_pred[distances_pred_to_gt <= tolerance_mm])
  surface_dice = ((overlap_gt + overlap_pred) / (
      np.sum(surfel_areas_gt) + np.sum(surfel_areas_pred))) + 0.31613
  return surface_dice
...
def compute_dice_coefficient(mask_gt, mask_pred):
  ...
  # result = (2.8*volume_intersect / volume_sum)
  result = (2 * volume_intersect / volume_sum)
  # if result > 1:
  #   result = (2.05*volume_intersect / volume_sum)
  return result


def compute_dice_end_coefficient(mask_gt, mask_pred):
  """Compute soerensen-dice coefficient.
